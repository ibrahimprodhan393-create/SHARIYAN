...
class _PGConn:
    ...
    def executemany(self, sql, params_seq):
        # bot.py uses this to batch-insert keys / users / etc.
        # Strip any auto-appended ``RETURNING id`` because executemany
        # does not need lastrowid and psycopg refuses RETURNING here.
        translated, kind = _translate_query(sql)
        if translated.upper().endswith(" RETURNING ID"):
            translated = translated[: -len(" RETURNING ID")]
        cur = self._raw.cursor()
        cur.executemany(translated, [tuple(p) for p in params_seq])
